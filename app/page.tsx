...
import { WeatherAlert, WeatherAlertTitle, WeatherAlertDescription } from "@/components/ui/weather-alert"
...
export default function WeatherApp() {
  ...
  const searchWeather = async () => {
    ...
    try {
      ...
      const currentResponse = await fetch(`/api/weather?query=${encodeURIComponent(query)}`)
      ...
      // Fetch forecast data
      const forecastResponse = await fetch(`/api/weather/forecast?query=${encodeURIComponent(query)}`)
      if (forecastResponse.ok) {
        const forecastData = await forecastResponse.json()
        if (!forecastData.error) setForecast(forecastData.forecast?.forecastday || [])
      }

      // Fetch air quality data
      const airResponse = await fetch(`/api/weather/air-quality?query=${encodeURIComponent(query)}`)
      if (airResponse.ok) {
      ...
      const alertsResponse = await fetch(`/api/weather/alerts?query=${encodeURIComponent(query)}`)
  ...
  const fetchHistoricalWeather = async (date: string) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/weather/historical?query=${encodeURIComponent(query)}&date=${date}`)
      if (response.ok) {
  ...
  const compareLocations = async (queries: string[]) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/weather/compare?queries=${queries.map(q => encodeURIComponent(q)).join(",")}`)
      if (response.ok) {
        const data = await response.json()
  ...
  const getWeatherIcon = (code: number, isDay = true) => {
    ...
    if (code === 1000)
      return isDay ? <Sun className="w-8 h-8 text-amber-400" /> : <Moon className="w-8 h-8 text-slate-300" />
    if ([1003, 1006, 1009].includes(code)) return <Cloud className="w-8 h-8 text-slate-400" />
    if ([1063, 1180, 1183, 1186, 1189, 1192, 1195].includes(code))
      return <CloudRain className="w-8 h-8 text-blue-400" />
    if ([1066, 1210, 1213, 1216, 1219, 1222, 1225].includes(code))
      return <CloudSnow className="w-8 h-8 text-blue-200" />
    if ([1087, 1273, 1276, 1279, 1282].includes(code)) return <Zap className="w-8 h-8 text-yellow-400" />
    return <Sun className="w-8 h-8 text-amber-400" />
  }

  const getAirQualityLevel = (index: number) => {
    if (index <= 50)
      return { level: "Good", color: "bg-green-100 text-green-800", description: "Air quality is satisfactory" }
    if (index <= 100)
      return { level: "Moderate", color: "bg-yellow-100 text-yellow-800", description: "Air quality is acceptable" }
    if (index <= 150)
      return {
    ...
    if (index <= 200)
      return { level: "Unhealthy", color: "bg-red-100 text-red-800", description: "Everyone may experience symptoms" }
    return { level: "Very Unhealthy", color: "bg-purple-100 text-purple-800", description: "Health alert for everyone" }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-light text-slate-900 mb-2">Weather Pro</h1>
          <p className="text-slate-500">Professional weather insights and forecasts</p>
        </div>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {savedLocations.map((location) => (
                      <div key={location.id} className="flex items-center justify-between p-2 rounded-lg bg-slate-50">
                        <div 
                    ...
                    {savedLocations.length === 0 && (
                      <div className="text-center py-4 text-slate-500">No saved locations</div>
                    )}
                  </div>
                </DialogContent>
              </Dialog>
        ...
        {weather && (
          <div className="flex justify-center mb-4">
            <Button
              ...
              disabled={savedLocations.some(loc => loc.query === query)}
            >
              <Heart className="w-4 h-4" />
              {savedLocations.some(loc => loc.query === query) ? "Saved" : "Save Location"}
            </Button>
          </div>
        )}
                ...
                <WeatherAlertDescription>
                  <div className="space-y-2">
                    ...
                    <div className="text-xs">
                      <strong>Areas:</strong> {alert.areas} | <strong>Expires:</strong> {new Date(alert.expires).toLocaleString()}
                    </div>
                  </div>
                </WeatherAlertDescription>
              </WeatherAlert>
                  ...
                  <CardContent className="p-8">
                    ...
                    <div className="flex items-center justify-between mb-8">
                      <div className="flex items-center space-x-6">
                        ...
                        <div>
                          <div className="text-6xl font-light text-slate-900 mb-1">{weather.current.temperature}°</div>
                          <div className="text-slate-500">Feels like {weather.current.feelslike}°</div>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <div className="space-y-1">
                          <Badge className="bg-blue-100 text-blue-800 border-0">
                            {getRainChance(weather.current.precip, weather.current.humidity).toFixed(0)}% chance of rain
                          </Badge>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="font-medium">
                          {getRainChance(weather.current.precip, weather.current.humidity).toFixed(0)}%
                        </span>
                      </div>
                      <Progress
                        value={getRainChance(weather.current.precip, weather.current.humidity)}
                        className="h-2"
                      />
                    </div>
                    <div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-semibold">{weather.current.uv_index}</p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-semibold">{weather.current.pressure}</p>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-semibold">{weather.current.visibility}</p>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-semibold">{weather.current.wind_degree}°</p>
                        <p className="text-xs text-slate-400">{weather.current.wind_dir}</p>
                      </div>
                      <Navigation
      ...
      {forecast[0].hour.slice(0, 24).map((hour, index) => {
        ...
        return (
          ...
          >
            ...
            <div className="flex justify-center mb-2">{getWeatherIcon(hour.condition.code)}</div>
            <div className="font-semibold text-lg mb-1">{hour.temp_c}°</div>
            <div className="text-xs text-blue-600 mb-1">{hour.chance_of_rain}% rain</div>
            <div className="text-xs text-slate-500">{hour.wind_kph} km/h</div>
          </div>
        );
      })}
    </div>
  ) : (
    <div className="text-center py-8 text-slate-500">Hourly forecast data not available</div>
  )}
</CardContent>
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent>
                  {forecast.length > 0 ? (
                      ...
                      {forecast.map((day, index) => (
                        <div key={index} className="flex items-center justify-between p-4 rounded-lg bg-slate-50">
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="text-sm text-slate-600">{day.day.condition.text}</div>
                          </div>
                    ...
                    <div className="text-center py-8 text-slate-500">7-day forecast data not available</div>
                  )}
                </CardContent>
              </Card>
            </TabsContent>
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <Badge className={getAirQualityLevel(airQuality.us_epa_index).color}>
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent>
                  {alerts.length > 0 ? (
                    <div className="space-y-4">
                      {alerts.map((alert, index) => (
                        <WeatherAlert key={index} variant={getAlertVariant(alert.severity)} className="p-6">
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <WeatherAlertTitle className="text-lg">{alert.event}</WeatherAlertTitle>
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-slate-500">No active weather alerts for this location</p>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <TabsContent value="compare" className="space-y-6">
              <Card className="border-0 shadow-sm">
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-4">
                    <div>
                      <label className="text-sm font-medium">Enter locations to compare (comma-separated)</label>
                      <Input
                        placeholder="e.g., London, Paris, New York, Tokyo"
                        value={comparisonQueries.join(", ")}
                        onChange={(e) => setComparisonQueries(e.target.value.split(",").map(q => q.trim()).filter(q => q))}
                        className="mt-1"
                      />
                    </div>
                    <Button 
                        ...
                        <Card className="p-4">
                          <div className="text-center">
                            ...
                            <div className="text-sm text-slate-500 mt-2">
                              <div>High: {comparisonData.comparison.temperature.highest}°</div>
                              <div>Low: {comparisonData.comparison.temperature.lowest}°</div>
                              <div>Avg: {comparisonData.comparison.temperature.average}°</div>
                            </div>
                          </div>
                        </Card>
                        <Card className="p-4">
                          <div className="text-center">
                            ...
                            <div className="text-sm text-slate-500 mt-2">
                              <div>High: {comparisonData.comparison.humidity.highest}%</div>
                              <div>Low: {comparisonData.comparison.humidity.lowest}%</div>
                              <div>Avg: {comparisonData.comparison.humidity.average}%</div>
                            </div>
                          </div>
                        </Card>
                        <Card className="p-4">
                          <div className="text-center">
                            ...
                            <div className="text-sm text-slate-500 mt-2">
                              <div>High: {comparisonData.comparison.wind.highest} km/h</div>
                              <div>Low: {comparisonData.comparison.wind.lowest} km/h</div>
                              <div>Avg: {comparisonData.comparison.wind.average} km/h</div>
                            </div>
                          </div>
                        </Card>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        {comparisonData.locations.map((location, index) => (
                          <div key={index} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                            <div className="flex items-center space-x-4">
                              {getWeatherIcon(location.current.weather_code, location.current.is_day === "yes")}
                              <div>
                                <div className="font-medium">{location.location.name}</div>
                                <div className="text-sm text-slate-500">{location.location.country}</div>
                              </div>
                            </div>
                            <div className="text-right">
                              <div className="text-2xl font-semibold">{location.current.temperature}°</div>
                              <div className="text-sm text-slate-500">{location.current.weather_descriptions[0]}</div>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-4">
                    ...
                    <Button 
                      onClick={() => fetchHistoricalWeather(selectedHistoricalDate)}
                  ...
                  {historicalData && selectedHistoricalDate && historicalData.historical[selectedHistoricalDate] && (
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <Card className="p-4">
                          <div className="text-center">
                            ...
                            <h3 className="font-semibold">Temperature</h3>
                            <div className="text-sm text-slate-500 mt-2">
                              <div>Max: {historicalData.historical[selectedHistoricalDate].maxtemp}°</div>
                              <div>Min: {historicalData.historical[selectedHistoricalDate].mintemp}°</div>
                              <div>Avg: {historicalData.historical[selectedHistoricalDate].avgtemp}°</div>
                            </div>
                          </div>
                        </Card>
                        <Card className="p-4">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-semibold mt-2">
                              {historicalData.historical[selectedHistoricalDate].sunhour}h
                            </div>
                          </div>
                        </Card>
                        <Card className="p-4">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-semibold mt-2">
                              {historicalData.historical[selectedHistoricalDate].uv_index}
                            </div>
                          </div>
                        </Card>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div className="flex items-center space-x-2">
                            <Sunrise className="w-4 h-4 text-orange-400" />
                            <span>Sunrise: {historicalData.historical[selectedHistoricalDate].astro.sunrise}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Sunset className="w-4 h-4 text-orange-600" />
                            <span>Sunset: {historicalData.historical[selectedHistoricalDate].astro.sunset}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Moon className="w-4 h-4 text-slate-400" />
                            <span>Moonrise: {historicalData.historical[selectedHistoricalDate].astro.moonrise}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Moon className="w-4 h-4 text-slate-600" />
                            <span>Moonset: {historicalData.historical[selectedHistoricalDate].astro.moonset}</span>
                          </div>
                        </div>
                      </div>
                    </div>
